...
int main(void)
{
  ...
  init();
  MW_Arduino_Init();
  rtmSetErrorStatus(velocity_control_M, 0);
  ...
  velocity_control_initialize();
  cli();
  sei();

  /* External Mode initialization */
  errorCode = extmodeInit(velocity_control_M->extModeInfo, &rtmGetTFinal
    (velocity_control_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(velocity_control_M, true);
    }
  }
  ...
  runModel = !extmodeSimulationComplete() && !extmodeStopRequested() &&
    !rtmGetStopRequested(velocity_control_M);
