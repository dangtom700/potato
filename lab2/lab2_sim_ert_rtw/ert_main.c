...
void rt_OneStep(void)
{
  ...
  currentTime = (extmodeSimulationTime_T) lab2_sim_M->Timing.clockTick0;
  lab2_sim_step();
...
int main(void)
{
  ...
  init();
  MW_Arduino_Init();
  rtmSetErrorStatus(lab2_sim_M, 0);
  ...
  errorCode = extmodeInit(lab2_sim_M->extModeInfo, &rtmGetTFinal(lab2_sim_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(lab2_sim_M, true);
    }
  }
  ...
  runModel = !extmodeSimulationComplete() && !extmodeStopRequested() &&
    !rtmGetStopRequested(lab2_sim_M);
